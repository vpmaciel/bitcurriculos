...
class PHPlot
{
    ...
    /**
     * Where to draw plot borders.
     * Can be scalar or array of choices.
     */
    ...
    protected static $plots = array(
        ...
        'candlesticks' => array(
            ...
            'draw_arg' => array(
                TRUE,
                FALSE
            ), // Draw candlesticks, only fill if "closed down"
        ...
        'stackedarea' => array(
            ...
            'draw_arg' => array(
                TRUE
            ), // Tells DrawArea to draw stacked area plot
            'sum_vals' => TRUE,
            'abs_vals' => TRUE
        ),
        'stackedbars' => array(
        ...
        'stackedsquaredarea' => array(
            ...
            'draw_arg' => array(
                TRUE
            ), // Tells DrawSquaredArea the data is cumulative
            'sum_vals' => TRUE,
            'abs_vals' => TRUE
        ),
        'thinbarline' => array(
     ...
     *            Path for output file. Omit, or NULL, or '' to mean no output file
     * @param string $input_file
     *            Path to a file to be used as background. Omit, NULL, or '' for none
     */
     ...
     *
     * @param string $imagecreate_function
     *            GD function to use: imagecreate or imagecreatetruecolor
     * @param int $width
     *            Image width in pixels
     * @param int $height
     *            Image height in pixels
     ...
     *            Filename of the image file to read
     * @param int $width
     *            Reference variable for width of the image in pixels
     * @param int $height
     *            Reference variable for height of the image in pixels
     ...
     *            Pathname to the image file to use as a background
     ...
     *
     * @param int[] $color
     *            Color specification as (R, G, B, A), or unset variable
     * @param int $default_color_index
     *            An already-allocated GD color index to use if $color is unset
     ...
     *            Array of color specifications, each an array (R,G,B,A)
     * @param int $max_colors
     *            Limit color allocation to no more than this number of colors
     ...
     *
     * @param int[] $color
     *            Color specification as (R, G, B, A)
    ...
    protected function SetDefaultStyles()
    {
        ...
        $this->SetImageBorderColor(array(
            194,
            194,
            194
        ));
     ...
     *            Color name or spec (#rrggbb, (r,g,b) array, etc)
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *            Color to make transparent; empty|omit|NULL to reset to none
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param array|string $which_color_array
     *            Color map array (name=>(R,G,B[,A]), or keyword small | large
     * @return bool True always
     */
    function SetRGBArray($which_color_array)
    {
        if (is_array($which_color_array)) { // User defined array
            ...
            if (! @include ('rgb.inc.php')) {
                return $this->PrintError("SetRGBArray(): Large color map could not be loaded " . "from 'rgb.inc.php'.");
            }
     ...
     *
     * @param string|int[] $color_asked
     *            Color spec to parse (color name, #rrggbb, (r,g,b) array, etc)
     * @param int $alpha
     *            Optional default alpha value (0-127, default 0 for opaque)
     ...
     *
     * @param array|string $which_data
     *            Array of color specifications, or one color, or empty
     * @param array|string $which_border
     *            Data border colors, deprecated - use SetDataBorderColors() instead
     * @param int $alpha
     *            Default alpha to apply to all data colors that do not have an alpha value
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $which_style
     *            Dashed line specification, in the form <pixels_on>-<pixels_off>...
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param int $which_ndxcol
     *            Color index to be used for drawing
     * @param bool $use_style
     *            TRUE or omit for dashed lines, FALSE for solid lines
     ...
     *
     * @param int[]|int $which_lw
     *            Array of line widths in pixels, or a single value to use for all data sets
     * @return bool True always
     */
     ...
     *
     * @param string[]|string $which_ls
     *            Array or single keyword: solid | dashed | none
     * @return bool True always
     */
    function SetLineStyles($which_ls = NULL)
    {
        if (is_array($which_ls)) {
            ...
            $this->line_styles = ($which_ls) ? array(
                $which_ls
            ) : array(
                'solid'
            );
     ...
     *
     * @param int $which_spc
     *            Text line spacing factor (pixels for GD text, scale control for TTF text)
     * @return bool True always
     */
     ...
     *
     * @param bool $which_ttf
     *            True to default to TrueType fonts, False to default to GD (fixed) fonts
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *            Font filename or path; omit or NULL to use a default font
     * @return bool True (False on error if an error handler returns True)
     */
    ...
    protected function GetDefaultTTFont()
    {
        if (! isset($this->default_ttfont)) {
            ...
            $fonts = array(
                'LiberationSans-Regular.ttf', // For Linux with a correct GD font search path
                'Verdana.ttf',
                'Arial.ttf',
                'Helvetica.ttf', // For Windows, maybe others
     ...
     *            Name of the element to change the font for
     * @param int|string $which_font
     *            GD font number: 1 2 3 4 or 5
     * @param int $which_spacing
     *            Optional spacing in pixels between text lines
     * @return bool True (False on error if an error handler returns True)
     * @since 5.0.6
     */
     ...
     *            Name of the element to change the font for
     * @param string $which_font
     *            TrueType font file or pathname; empty or NULL for the default font
     * @param int $which_size
     *            Optional font size in points (default 12)
     * @param int $which_spacing
     *            Optional line spacing adjustment factor
     * @return bool True (False on error if an error handler returns True)
     * @since 5.0.6
     */
     ...
     *            Name of the element to change the font for
     * @param int|string $which_font
     *            For GD fonts, a font number; for TrueType, a font filename or pathname
     * @param int $which_size
     *            Optional font size in points for TrueType fonts, ignored for GD fonts
     ...
     *
     * @param array $font
     *            The font, specified as a PHPlot font array
     ...
     *
     * @param bool $draw_it
     *            True to draw the text, False to just return the orthogonal width and height
     * @param array $font
     *            A PHPlot font array (with 'ttf' = False)
     * @param float $angle
     *            Text angle in degrees. GD only supports 0 and 90.
     * @param int $x
     *            Reference point X coordinate for the text (ignored if $draw_it is False)
     * @param int $y
     *            Reference point Y coordinate for the text (ignored if $draw_it is False)
     * @param int $color
     *            GD color index to use for drawing the text (ignored if $draw_it is False)
     * @param string $text
     *            The text to draw or size (can have newlines \n within)
     * @param float $h_factor
     *            Horizontal alignment factor: 0=left|0.5=center|1=right (ignored if !$draw_it)
     * @param float $v_factor
     *            Vertical alignment factor: 0=top|0.5=center|1=bottom (ignored if !$draw_it)
     ...
     *            True to draw the text, False to just return the orthogonal width and height
     * @param array $font
     *            A PHPlot font array (with 'ttf' = True)
     * @param float $angle
     ...
     * @param bool $draw_it
     *            True to draw the text, False to just return the orthogonal width and height
     * @param string|array|null $font_id
     *            Text element name, empty or NULL for 'generic', or font array
     * @param float $angle
     *            Text angle in degrees
     * @param int $x
     *            Reference point X coordinate for the text (ignored if $draw_it is False)
     ...
     *            The text to draw or size (can have newlines \n within)
     * @param string $halign
     *            Horizontal alignment: left | center | right (ignored if $draw_it is False)
     * @param string $valign
     *            Vertical alignment: top | center | bottom (ignored if $draw_it is False)
     * @return bool|int[] True, if drawing text; an array of ($width, $height) if not.
     */
     ...
     *
     * @param string|array|null $which_font
     *            Text element name, empty or NULL for 'generic', or font array
     * @param float $which_angle
     *            Text angle in degrees
     * @param int $which_xpos
     *            Reference point X coordinate for the text
     * @param int $which_ypos
     *            Reference point Y coordinate for the text
     * @param int $which_color
     *            GD color index to use for drawing the text
     * @param string $which_text
     *            The text to draw, with newlines (\n) between lines
     * @param string $which_halign
     *            Horizontal alignment (relative to the image): left | center | right
     * @param string $which_valign
     *            Vertical alignment (relative to the image): top | center | bottom
     * @return bool True always
     */
     ...
     *
     * @param string $format
     *            The format to use: jpg | png | gif | wbmp
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $input_file
     *            Path to the file to be used (jpeg, png or gif)
     * @param string $mode
     *            Optional method for the background: centeredtile | tile | scale
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param bool $which_ii
     *            True to suppress HTTP headers, False to include the headers
     * @return bool True always
     */
     ...
     *            Reference variable where the MIME type is returned
     * @param string $output_f
     *            Reference variable where the GD output function name is returned
     * @return bool True (False on error if an error handler returns True)
     * @since 5.5.0
     */
     ...
     *            Optional encoding to use: dataurl (default) | raw | base64
     ...
     *
     * @param string $text
     *            Text of the message to display in the image
     * @param string[] $options
     *            Optional associative array of control options
     * @return bool True always
     * @since 5.7.0
     */
     ...
     *
     * @param bool $error_image
     *            True to enable the error image, False to disable it.
     * @return bool True always
     * @since 5.5.0
     */
     ...
     *
     * @param string $which_xdlp
     *            Desired label position: plotdown | plotup | both | none | plotin | plotstack
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $which_ydlp
     *            Desired label position: plotleft | plotright | both | none | ...
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *            Desired label position: plotdown | plotup | both | none | xaxis
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $which_ytlp
     *            Desired label position: plotleft | plotright | both | none | yaxis
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $mode
     *            Which label type to configure: x | y | xd | yd | p
     * @param array $args
     *            Additional arguments controlling the format type
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $type
     *            Formatting mode: data | time | printf | custom, or '' to reset to default
     * @param mixed $varargs
     *            One or more additional arguments that depend on $type
     * @return bool True (False on error if an error handler returns True)
     */
    function SetXLabelType($type = NULL, $varargs = NULL)
    { // Variable arguments
        $args = func_get_args();
     ...
     * @since 5.1.0
     */
    function SetXDataLabelType($type = NULL, $varargs = NULL)
    { // Variable arguments, named params are unused
        $args = func_get_args();
     ...
     * @return bool True (False on error if an error handler returns True)
     */
    function SetYLabelType($type = NULL, $varargs = NULL)
    { // Variable arguments, named params are unused
        $args = func_get_args();
     ...
     * @since 5.1.0
     */
    function SetYDataLabelType($type = NULL, $varargs = NULL)
    { // Variable arguments, named params are unused
        $args = func_get_args();
     ...
     *
     * @param string|string[] $source_
     *            Label source keyword or array: percent | value | label | index | ''
     * @param string $type
     *            Optional formatting mode: data | printf | custom
     * @param mixed $varargs
     *            Zero or more additional arguments telling how to format the label
     * @return bool True (False on error if an error handler returns True)
     * @since 5.6.0
     */
    function SetPieLabelType($source_, $type = NULL, $varargs = NULL)
    { // Variable arguments, named params unused
        $args = func_get_args();
        ...
        if (empty($source)) {
            ...
            $args = array(
                ''
            ); // See below - tells SetLabelType to do no formatting or default.
        } else {
     ...
     *
     * @param string $which_xtf
     *            Formatting string to use (see PHP function strftime())
     * @return bool True always
     */
     ...
     *
     * @param string $which_ytf
     *            Formatting string (see PHP function strftime())
     * @return bool True always
     */
     ...
     *            The character to use as a thousands grouping separator
     * @return bool True always
     * @since 5.0.4
     */
     ...
     *
     * @param string $which_opt
     *            String to check, usually the provided value of a function argument
     * @param string $which_acc
     *            String of accepted choices, lower-case, choices separated by comma space
     * @param string $which_func
     *            Name of the calling function, for error messages, usually __FUNCTION__
     ...
     *            Valid data types, separated by comma space
    ...
    protected function CheckDataType($valid_types)
    {
        ...
        $this->PrintError("Data type '$this->data_type' is not valid for '$this->plot_type' plots." . " Supported data type(s): '$valid_types'");
        return FALSE;
    }
     ...
     *
     * @param bool $which_browser_cache
     *            True to allow the browser to cache the image, false to not allow
     * @return bool True always
     */
     ...
     *
     * @param bool $which_pi
     *            True to have DrawGraph() call PrintImage() when done, false to not output
     * @return bool True always
     */
     ...
     *
     * @param string|string[] $pdt
     *            Border control keyword(s): left|right|top|bottom|sides|none|full
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $sibt
     *            Border type: raised | solid | plain | none
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param bool $dpab
     *            True to draw the plot area background color, false to not draw it
     * @return bool True always
     */
     ...
     *
     * @param bool $dxg
     *            True to draw the X grid lines, false to not draw them; or NULL to restore default
     * @return bool True always
     */
     ...
     *
     * @param bool $dyg
     *            True to draw the Y grid lines, false to not draw them; or NULL to restore default
     * @return bool True always
     */
     ...
     *
     * @param bool $ddg
     *            True to draw the grid with dashed lines, false to use solid lines
     * @return bool True always
     */
     ...
     *
     * @param bool $dxdl
     *            True to draw the X data label lines, false to not draw them
     * @return bool True always
     */
     ...
     *
     * @param bool $dydl
     *            True to draw the Y data label lines, false to not draw them
     * @return bool True always
     * @since 6.0.0
     */
     ...
     *
     * @param bool $dpb
     *            True to draw the pie chart segment borders, false to not draw them
     * @return bool True always
     * @since 6.0.0
     */
     ...
     *
     * @param string $which_title
     *            The text to use for the main plot title. Can contain multiple lines
     * @return bool True always
     */
     ...
     *
     * @param string $which_xtitle
     *            The text string to use for the X axis title. Can contain multiple lines
     * @param string $which_xpos
     *            Optional position for the X axis and title: plotdown plotup both none
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *            The text string to use for the Y axis title. Can contain multiple lines
     * @param string $which_ypos
     *            Optional position for the X axis and title: plotleft plotright both none
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $which_pt
     *            The plot type, such as bars, lines, pie, ...
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param float $pos
     *            Axis position as an integer Y world coordinate; '' or omit for default
     * @return bool True always
     */
     ...
     *
     * @param float $pos
     *            Axis position as an integer X world coordinate; '' or omit for default
     * @return bool True always
     */
     ...
     *
     * @param bool $draw
     *            True to draw the axis line, False to not draw it
     * @return bool True always
     * @since 5.3.0
     */
     ...
     *
     * @param string $which_xst
     *            The scale type: linear | log
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param float $which_blp
     *            Label position factor (0 <= blp <= 1); 0 or False for no labels
     * @return bool True always
     */
     ...
     *
     * @param int $which_ebs
     *            Length in pixels of the error bar "T"
     * @return bool True always
     */
     ...
     *
     * @param string $which_ebs
     *            Error bar shape: tee | line
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string|string[] $which_pt
     *            Array (or single value) of valid point shapes
     * @return bool True (False on error if an error handler returns True)
     */
    function SetPointShapes($which_pt)
    {
        $this->point_shapes = $this->CheckOptionArray($which_pt, 'halfline, line, plus, cross, rect,' . ' circle, dot, diamond, triangle, trianglemid, delta, yield, star, hourglass,' . ' bowtie, target, box, home, up, down, none', __FUNCTION__);
     ...
     * that uses points. See CheckPointParams()
     *
     * @param string[]|string $which_ps
     *            Array (or single value) of point sizes in pixels
     * @return bool True always
     */
     ...
     *
     * @param bool $bl
     *            True to break the lines, false to connect around missing data
     * @return bool True always
     */
     ...
     *
     * @param string $which_dt
     *            The data array format type: text-data | data-data | ...
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *            The data array, an array of row arrays, interpreted per SetDataType()
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *            Reference variable for the array (or scalar) to pad
     * @param int $size
     *            Minimum size of the resulting array
     */
     ...
     *
     * @param string $reason
     *            A pre-defined name where a callback can be defined
     * @param callback $function
     *            Function or method to register for callback
     * @param mixed $arg
     *            Optional opaque argument to supply to the callback function when called
     ...
     *
     * @param string $reason
     *            A string naming one of the pre-defined callback reasons
     * @param mixed $varargs
     *            Zero or more additional arguments to be passed to the callback
     ...
     *
     * @param float $sin_t
     *            sin() of the angle of the text offset from a reference point
     * @param float $cos_t
     *            cos() of the angle of the text offset from a reference point
     * @param string $h_align
     *            Reference variable to get the horizontal alignment, for DrawText()
     * @param string $v_align
     *            Reference variable to get the vertical alignment, for DrawText()
     * @param bool $reverse
     *            True to reverse the alignment, e.g. for text inside an ellipse
     * @since 5.6.0
     */
     ...
     *
     * @param string $label_control
     *            Label position control; either $x_data_label_pos or $y_data_label_pos
     * @param array $dvl
     *            Reference argument for result parameters for DrawDataValueLabel()
     ...
     *
     * @param bool $enable
     *            True to enable automatic size calculation, False to use the maximum area
     * @return bool True always
     * @since 5.6.0
     */
     ...
     *
     * @param string $which
     *            Direction for pie segments: clockwise | cw | counterclockwise | ccw
     * @return bool True (False on error if an error handler returns True)
     * @since 6.0.0
     */
        ...
        for ($i = 0; $i < $this->num_data_rows; $i ++) {
            ...
            if ($sum_vals) {
                $all_dv = array(
                    0,
                    0
                ); // One limit is 0, other calculated below
            } else {
                $all_dv = array();
            }
            while ($j < $n_vals) {
     ...
     *
     * @param bool $maximize
     *            If True, use full image area (less margins and title space)
     * @return bool True always
     */
     ...
     *
     * @param int $which_lm
     *            Left margin in pixels; omit or NULL to auto-calculate
     * @param int $which_rm
     *            Right margin in pixels; omit or NULL to auto-calculate
     * @param int $which_tm
     *            Top margin in pixels; omit or NULL to auto-calculate
     * @param int $which_bm
     *            Bottom margin in pixels; omit or NULL to auto-calculate
     * @return bool True always
     */
     ...
     *
     * @param int $x1
     *            Top left corner X coordinate in pixels; omit or NULL to auto-calculate
     * @param int $y1
     *            Top left corner Y coordinate in pixels; omit or NULL to auto-calculate
     * @param int $x2
     *            Bottom right corner X coordinate in pixels; omit or NULL to auto-calculate
     * @param int $y2
     *            Bottom right corner Y coordinate in pixels; omit or NULL to auto-calculate
     * @return bool True always
     */
     ...
     *
     * @param float $range
     *            The data range (max - min), already checked as > 0
     * @param int $min_ticks
     *            Smallest number of intervals allowed, already checked > 0
     ...
     *
     * @param string $which
     *            Which axis to calculate for. Must be 'x' or 'y'
     * @param float $range
     *            The data range (max - min), already checked as > 0
     ...
     *
     * @param float $plot_limit
     *            Reference to (possibly unset) plot_min_[xy] or plot_max_[xy]
     * @param bool $implied
     *            True if this is the implied variable (X for vertical plots)
     * @param float $data_limit
     *            Actual data limit at this end: one of min_x, max_x, etc.
     ...
     *
     * @param string $which
     *            Which axis: 'x' or 'y', used only for reporting
     * @param float $plot_min
     *            Reference variable for the low end limit, changed if necessary
     * @param float $plot_max
     *            Reference variable for the high end limit, changed if necessary
     * @param bool $adjust_min
     *            True means $plot_min was auto-calculated, and may be adjusted
     * @param bool $adjust_max
     *            True means $plot_max was auto-calculated, and may be adjusted
     * @return bool True (False on error if an error handler returns True)
     * @since 6.0.0
     */
     ...
     *            Which axis to calculate for. Must be 'x' or 'y'
     * @param float $adjust
     *            Reference variable for the range end adjustment factor, NULL or already set
     * @since 6.0.0
     */
    ...
    protected function CalcPlotRange($which)
    {
        ...
        if ($adjust_min && $plot_min > 0 && $zero_magnet > 0 && ($zero_magnet == 1.0 || $plot_max / $range < $zero_magnet / (1 - $zero_magnet))) {
            $plot_min = 0;
            $range = $plot_max;
        }

        // Similar to above, but for negative data: zero magnet pulls max up to zero:
        if ($adjust_max && $plot_max < 0 && $zero_magnet > 0 && ($zero_magnet == 1.0 || - $plot_min / $range < $zero_magnet / (1 - $zero_magnet))) {
            $plot_max = 0;
            $range = 0 - $plot_min;
        }
    ...
    protected function CalcPlotAreaWorld()
    {
        ...
        if ($this->GetCallback('debug_scale')) {
            $this->DoCallback('debug_scale', __FUNCTION__, array(
                'plot_min_x' => $this->plot_min_x,
                'plot_min_y' => $this->plot_min_y,
                'plot_max_x' => $this->plot_max_x,
                'plot_max_y' => $this->plot_max_y,
                'x_tick_inc' => $this->x_tick_inc,
                'y_tick_inc' => $this->y_tick_inc
            ));
        }
     ...
     *
     * @param float $xmin
     *            X data range minimum; omit or NULL to auto-calculate
     * @param float $ymin
     *            Y data range minimum; omit or NULL to auto-calculate
     * @param float $xmax
     *            X data range maximum; omit or NULL to auto-calculate
     * @param float $ymax
     *            Y data range maximum; omit or NULL to auto-calculate
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param bool $stacked
     *            If true, this is a stacked bar plot (1 bar per group)
     * @param bool $verticals
     *            If false, this is a horizontal bar plot
     * @return bool True always
     */
    ...
    protected function CalcTranslation()
    {
        ...
        if ($this->GetCallback('debug_scale')) {
            $this->DoCallback('debug_scale', __FUNCTION__, array(
                'xscale' => $this->xscale,
                'yscale' => $this->yscale,
                'plot_origin_x' => $this->plot_origin_x,
     ...
     *
     * @param float $x_world
     *            X world coordinate value to translate
     ...
     *
     * @param float $y_world
     *            Y world coordinate value to translate
     ...
     *
     * @param string $which
     *            Which axis to calculate tick parameters for. Must be 'x' or 'y'
     ...
     *
     * @param string $which_pos
     *            Which format controls to use: x | y | xd | yd | p (tick, data, pie)
     * @param mixed $which_lab
     *            The value of the label to be formatted (usually a float)
     * @param mixed $varargs
     *            Zero or more additional arguments to pass to a custom format function
     * @return string The formatted label value (original value if no formatting is enabled)
     */
    protected function FormatLabel($which_pos, $which_lab, $varargs = NULL)
        ...
        if ($which_lab !== '' && ! empty($format['type'])) {
            switch ($format['type']) {
                case 'title': // Note: This is obsolete
                    ...
                    break;
                case 'data':
                    $which_lab = $format['prefix'] . $this->number_format($which_lab, $format['precision']) . $this->data_units_text . // Obsolete
                    $format['suffix'];
                    break;
                case 'time':
     ...
     *
     * @param string $which
     *            Which axis to adjust parameters for: 'x' or 'y'
     * @param float $zero_magnet
     *            Zero magnet value, NULL to ignore
     * @param string $adjust_mode
     *            Range extension mode, NULL to ignore
     * @param float $adjust_amount
     *            Range extension amount, NULL to ignore
     * @return bool True always
     * @since 6.0.0
     */
     ...
     *
     * @param float $zero_magnet
     *            Optional X axis zero magnet value, controlling range extension to include 0
     * @param string $adjust_mode
     *            Optional X axis range extension mode: T | R | I (Tick, Range, Integer)
     * @param float $adjust_amount
     *            Optional X axis range extension amount
     * @return bool True always
     * @since 6.0.0
     */
     ...
     *
     * @param float $zero_magnet
     *            Optional Y axis zero magnet value, controlling range extension to include 0
     * @param string $adjust_mode
     *            Optional Y axis range extension mode: T | R | I (Tick, Range, Integer)
     * @param float $adjust_amount
     *            Optional Y axis range extension amount
     * @return bool True always
     * @since 6.0.0
     */
     ...
     * @param string $tick_mode
     *            Tick increment calculation mode
     * @param bool $tick_inc_integer
     *            True to always use integer tick increments
     * @return bool True (False on error if an error handler returns True)
     * @since 6.0.0
     */
     ...
     *
     * @param int $min_ticks
     *            Minimum number of tick intervals along the axis
     * @param string $tick_mode
     *            Tick increment calculation mode: decimal | binary | date
     * @param bool $tick_inc_integer
     *            True to always use integer tick increments, false to allow fractions
     * @return bool True (False on error if an error handler returns True)
     * @since 6.0.0
     */
     ...
     *            X tick increment in world coordinates; omit or '' for default auto-calculate
     * @return bool True always
     */
     ...
     *
     * @param float $which_ti
     *            Y tick increment in world coordinates; omit or '' for default auto-calculate
     * @return bool True always
     */
     ...
     *
     * @param int $which_nt
     *            Number of X tick intervals; omit or '' for default auto-calculate
     * @return bool True always
     */
     ...
     *
     * @param int $which_nt
     *            Number of Y tick intervals; omit or '' for default auto-calculate
     * @return bool True always
     */
     ...
     *            Tick mark position: plotdown | plotup | both | xaxis | none
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param string $which_tp
     *            Tick mark position: plotleft | plotright | both | yaxis | none
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param bool $skip
     *            True to skip the tick mark and label; false to draw them
     * @return bool True always
     */
     ...
     *
     * @param int $which_xln
     *            Outer length of the tick marks, in pixels
     * @return bool True always
     */
     ...
     *
     * @param int $which_xc
     *            Crossing length of the tick marks, in pixels
     * @return bool True always
     */
     ...
     *
     * @param float $xta
     *            Tick anchor position in World Coordinates; omit or NULL for no anchor
     * @return bool True always
     * @since 5.4.0
     */
     ...
     *            True to do it even if already done, False or omit to do once only
     * @return bool True always
     */
     ...
     *
     * @param string $file
     *            Filename of the picture to be used as tile
     * @param int $xorig
     *            X device coordinate of where the tile is to begin
     * @param int $yorig
     *            Y device coordinate of where the tile is to begin
     * @param int $width
     *            Width of the area to be tiled
     * @param int $height
     *            Height of the area to be tiled
     * @param string $mode
     *            Tiling mode: centeredtile | tile | scale
     * @return bool True (False on error if an error handler returns True)
     *        
     */
     ...
     *
     * @param float $x
     *            X value for the label
     * @param int $x_pixels
     *            X device coordinate for this tick mark
     * @return bool True always
     * @since 5.0.5
     */
     ...
     *
     * @param float $y
     *            Y value for the label
     * @param int $y_pixels
     *            Y device coordinate for this tick mark
     * @return bool True always
     * @since 5.8.0
     */
     ...
     *
     * @param bool $draw_axes
     *            True or omit to draw the X axis and Y axis, false to not (for pie charts)
     * @return bool True always
     */
     ...
     *
     * @param string $x_or_y
     *            Which labels: x | y, used to select font, angle, formatting
     * @param int $row
     *            Identifies the row of the data point (for custom label formatting)
     * @param int $column
     *            Identifies the column of the data point (for custom label formatting)
     * @param float $x_world
     *            X world coordinate of the base point
     * @param float $y_world
     *            Y world coordinate of the base point
     * @param mixed $text
     *            The text to draw after formatting with FormatLabel()
     * @param mixed[] $dvl
     *            Associative array with additional label position controls (see above)
     ...
     * @param int $xpos
     *            X position for the label, in device coordinates
     * @param int $row
     *            Row index, 0 for the first X, 1 for the second, etc.
     * @param bool $do_lines
     *            True for plot types that support data label lines, False or omit if not
     * @return bool True always
     */
     ...
     *            Text of the label to draw
     * @param int $ypos
     *            Y position for the label, in device coordinates
     * @param int $row
     *            Row index, 0 for the first Y, 1 for the second, etc.
     * @param bool $do_lines
     *            True for plot types that support data label lines, False or omit if not
     * @return bool True always
     * @since 5.1.2
     ...
     *
     * @param int $xpos
     *            X position in pixels of the line
     * @param int $row
     *            Index of the data row being drawn
     * @return bool True always
     */
     ...
     *
     * @param int $ypos
     *            Y position in pixels of the line
     * @param int $row
     *            Index of the data row being drawn
     * @return bool True always
     * @since 6.0.0
     ...
     *
     * @param int $index
     *            Pie slice number, starting with 0
     * @param string[] $pie_label_source
     *            Label formatting mode(s). See CheckPieLabels() and SetPieLabelType()
     * @param float $arc_angle
     *            Delta angle for this slice, in degrees
     * @param float $slice_weight
     *            Numeric value, or relative weight, of this slice
     ...
     *
     * @param string $label_txt
     *            Pre-formatted label, from FormatPieLabel()
     * @param int $xc
     *            X device coordinate of the pie center
     * @param int $yc
     *            Y device coordinate of the pie center
     * @param float $start_angle
     *            Slice starting angle in degrees
     * @param float $arc_angle
     *            Slice angular width in degrees
     * @param mixed[] $r
     *            Additional paramter array (from DrawPieChart, see comment above)
     * @return bool True always
     * @since 5.6.0
     */
     ...
     *            Array of strings for the legend, or a single string to append
     * @return bool True always
     */
     ...
     *
     * @param int $which_x
     *            X coordinate in pixels of the upper left corner of the legend box
     * @param int $which_y
     *            Y coordinate in pixels of the upper left corner of the legend box
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param int $which_x
     *            World coordinate X of the upper left corner of the legend box
     * @param int $which_y
     *            World coordinate Y of the upper left corner of the legend box
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param float $x
     *            Relative coordinate X indicating an anchor point on the legend box
     * @param float $y
     *            Relative coordinate X indicating an anchor point on the legend box
     * @param string $relative_to
     *            Position is relative to: image | plot | world | title
     * @param float $x_base
     *            Base point X for positioning to the anchor point
     * @param float $y_base
     *            Base point Y for positioning to the anchor point
     * @param int $x_offset
     *            Additional legend box offset X value in pixels
     * @param int $y_offset
     *            Additional legend box offset Y value in pixels
     * @return bool True (False on error if an error handler returns True)
     * @since 5.4.0
     */
     ...
     *            How to align the text in the legend: left | right
     * @param string $colorbox_align
     *            How to align the color boxes or shape markers: left | right | none | ''
     * @return bool True (False on error if an error handler returns True)
     * @since 5.0.4
     */
     ...
     *
     * @param bool $use_shapes
     *            True to use shapes or lines, false to use color boxes
     * @return bool True always
     * @since 5.4.0
     */
    ...
    /**
     * Controls the order of text lines in the legend (e.g.
     * for Stackedbar plots)
     *
     * @param bool $reversal
     *            True to reverse the order - bottom up; False for default top down
     * @return bool True always
     * @since 5.5.0
     */
     ...
     *
     * @param string $cbbmode
     *            Color box mode: none | textcolor | databordercolor
     * @return bool True (False on error if an error handler returns True)
     * @since 6.0.0
     */
    ...
    protected function GetLegendPosition($width, $height)
    {
        ...
        switch ($mode) {
            ...
            case 'plot': // SetLegendPosition with mode='plot', relative coordinates over plot area.
                return array(
                    (int) ($x_base * $this->plot_area_width - $x * $width) + $this->plot_area[0] + $x_offset,
            ...
            case 'world': // User-defined position in world-coordinates (SetLegendWorld), using x_base, y_base
                return array(
                    $this->xtr($x_base) + $x_offset - (int) ($x * $width),
            ...
            case 'image': // SetLegendPosition with mode='image', relative coordinates over image area.
                ...
                return array(
                    (int) ($x_base * $this->image_width - $x * $width) + $x_offset,
     ...
     *
     * @param int $row
     *            Row index for the current data point (used only for a custom data color callback)
     * @param int $idx
     *            Column index for the current data point (e.g. 0 for the first Y value for each X)
     * @param mixed[] $vars
     *            Reference variable, array for local storage (caller makes an empty array)
     * @param int $data_color
     *            Reference variable: returned color index to use for drawing
     * @param mixed $extra
     *            Extra variable passed through to a data color callback
     * @since 5.2.0
     */
     ...
     *            Reference variable: returned color index to use for drawing
     * @param
     *            int error_color Reference variable: returned color index to use for the error bars
     * @param mixed $extra
     *            Extra variable passed through to a data color callback
     * @since 5.2.0
     */
     ...
     *            Reference variable, array for local storage (caller makes an empty array)
     * @param int $data_color
     *            Returned color index to use for the bar fill
     * @param int $shade_color
     *            Returned color index for the shading, NULL if shading is off
     * @param int $border_color
     *            Returned color index for the bar borders, NULL if borders are off
     * @since 5.2.0
     */
     ...
     *
     * @param int $x
     *            X device coordinate of the center of the shape
     * @param int $y
     *            Y device coordinate of the center of the shape
     * @param int $record
     *            Index into point_shapes[] and point_sizes[] arrays (taken modulo their sizes)
     * @param int $color
     *            Color index to use for the point shape
     * @param bool $allow_none
     *            True or omit allow 'none' as shape, False to substitute (for legend)
     * @return bool True always
     * @since 5.4.0
     */
     ...
     *
     * @param int $row
     *            Row index of data point (only used for a data_points callback)
     * @param int $column
     *            Column index of data point, used to select the marker shape and size
     * @param int $x_world
     *            X world coordinate of the data point
     * @param int $y_world
     *            Y world coordinate of the data point
     * @param int $color
     *            Color to use for the data point
     ...
     *            Row index of data point (only used for a data_points callback)
     * @param int $column
     *            Column index of data point (only used for a data_points callback)
     * @param int $x1
     *            One corner of the bar X (device coordinates)
     * @param int $y1
     *            One corner of the bar Y (device coordinates)
     * @param int $x2
     *            Other corner of the bar X (device coordinates)
     * @param int $y2
     *            Other corner of the bar Y (device coordinates)
     * @param int $data_color
     *            Color index to use for the bar fill
     * @param int $shade_color
     *            Color index to use for the shading (if shading is on, else NULL)
     * @param int $border_color
     *            Color index to use for the bar outlines (borders), if enabled, else NULL
     * @param bool $shade_top
     *            Shade the top? (Suppressed for downward stack segments except first)
     * @param bool $shade_side
     *            Shade the right side? (Suppressed for leftward stack segments except first)
     * @return bool True always
     * @since 5.2.0
     */
     ...
     *
     * @param float $x
     *            X world coordinate of the data point (dependent variable)
     * @param float $y
     *            Y world coordinate of the data point (independent variable)
     * @param float $error_plus
     *            X error offset above $x (world coordinates >= 0)
     * @param float $error_minus
     *            X error offset below $x (world coordinates >= 0)
     * @param int $color
     *            Color to use for the error bars
     * @return bool True always
     * @since 6.1.0
     */
     ...
     *
     * @param float $x
     *            X world coordinate of the data point (independent variable)
     * @param float $y
     *            Y world coordinate of the data point (dependent variable)
     * @param float $error_plus
     *            Y error offset above $y (world coordinates >= 0)
     * @param float $error_minus
     *            Y error offset below $y (world coordinates >= 0)
     * @param int $color
     *            Color to use for the error bars
     * @return bool True always
     * @since 6.1.0
    ...
    protected function CheckPieLabels()
    {
        if (empty($this->pie_label_source)) { // SetPieLabelType() was not called, or was reset to default
            ...
            return array(
                'percent'
            ); // Default to 'percent' labels (leaving this->pie_label_source empty)
        }
     ...
     *
     * @param bool $stacked
     *            True for a stacked plot (cumulative Y), false if not
     * @param int[] $xd
     *            Reference variable - returned array with X device coordinates
     * @param int[] $yd
     *            Reference variable - returned 2D array with Y device coordinates
     ...
     *
     * @param int $n_columns
     *            Number of columns in $yd, returned by SetupAreaPlot()
     * @param int[] $xd
     *            X device coordinate array from SetupAreaPlot()
     * @param int[] $yd
     *            Y device coordinate array from SetupAreaPlot()
     * @param bool $stacked
     *            True for a stacked plot (cumulative Y), false if not
     * @param bool $stepped
     *            True for a stepped (squared) plot, false for a regular area plot
     * @return bool True always
     * @since 6.2.0
     */
     ...
     *
     * @param bool $paired
     *            True if doing a linepoints plot, False or omit for points plot
     * @return bool True (False on error if an error handler returns True)
     */
    protected function DrawDots($paired = FALSE)
    {
        if (! $this->CheckDataType('text-data, data-data, text-data-yx, data-data-yx, ' . 'data-data-error, data-data-yx-error'))
            return FALSE;
     ...
     * be produced.
     *
     * @param bool $do_stacked
     *            True for stackedarea plot, false or omit for area plot
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param bool $paired
     *            True if doing a linepoints plots, False or omit for lines plot
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param bool $do_stacked
     *            True for cumulative (stacked), false or omit for unstacked.
     * @return bool True (False on error if an error handler returns True)
     */
     ...
     *
     * @param bool $draw_candles
     *            True to draw a candlestick plot, False to draw an ohlc plot
     * @param bool $always_fill
     *            True to always fill, False or omit to fill only when closing down
     * @return bool True (False on error if an error handler returns True)
     * @since 5.3.0
     */
